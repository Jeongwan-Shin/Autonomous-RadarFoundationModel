...
`train` 분할에서, 리그 프로필을 섞어서(`all_profiles=True`) 뽑았습니다. **전방
레이더가 있는 클립에서만 뽑았습니다** — 110건 전부 반사점이 있습니다.

그것이 데이터의 비율은 아닙니다. 전체 177,891 클립 중 **17,130건(9.6%)은 전방
레이더가 아예 없습니다.**

| 분할 | 클립 | 전방 레이더 없음 |
|---|---|---|
| train | 86,607 | 6,141 (7.1%) |
| val | 54,163 | 5,596 (10.3%) |
| test | 37,121 | 5,393 (14.5%) |
| 전체 | 177,891 | 17,130 (9.6%) |

리그별로는 `lrr1`/`mrr2` 가 각 73,564 클립(41.4%), `srr0` 가 87,197(49.0%)
입니다. 로더는 그런 클립에도 프레임과 ego 를 그대로 주고 레이더 텐서만 0 으로
채웁니다(`Sensors present: ... no radar`). 그 조건까지 예제로 보려면
`python -m notebooks.build_example_data --any-radar` 로 다시 만듭니다.

`test` 의 없음 비율이 `train` 의 두 배입니다. 레이더에 기대는 태스크에서
평가가 학습보다 불리한 쪽으로 기울어 있다는 뜻이라, 레이더 기여도를 읽을 때
염두에 둘 사실입니다.
