...
def main(argv=None):
    ...
    for clip_id, idxs in chosen:
        ...
        full = ds[whole]
        # 프레임은 캐시의 원본 크기(1920x1088)로 저장한다.
        #
        # 예전에는 20 프레임 태스크의 아이템에서 그대로 받아 적었는데, 그 태스크
        # 의 FRAME_SIZE 가 384x216 이라 번들이 384 로 굳었다. 검출은 1920 으로
        # 학습하므로 프레임당 비전 토큰이 1,020 이 아니라 42 가 되고, 모델은
        # 학습 때와 전혀 다른 축척의 그림을 본다 -- 측정하면 det F1 이 0.60 대
        # 에서 0.017 로 무너진다. 계획은 자차 상태 문장에 기대므로 멀쩡해 보여서,
        # 한 태스크만 보고 있으면 알아채지 못한다.
        #
        # 줄이는 것은 읽는 쪽에서 한다. 큰 것에서 줄일 수는 있어도 작은 것에서
        # 되살릴 수는 없다.
        from training.frame_cache import load as _cache_load
        try:
            images = _cache_load(clip_id)
        except Exception:
            images = full["frames"]
        frame_names = []
        for j, image in enumerate(images[:len(full["frames"])]):
            name = f"f{j:02d}.jpg"
