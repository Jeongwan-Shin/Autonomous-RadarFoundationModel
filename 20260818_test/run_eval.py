...
# 학습의 instruct_data.FRAME_SIZE 를 그대로 옮겨 적은 것. 저장소를 옮겨도
# 따라오도록 여기에 값으로 둔다 -- import 하면 데이터셋 전체가 딸려 온다.
DEFAULT_FRAME_SIZE = (384, 216)
FRAME_SIZE = {}
for _t in ("det_objects_azdeg", "det_objects_3dbbox",
           "plan_ego_xy", "plan_ego_control", "agent_traj_azdeg",
...
def load_item(root, rec, n_frames, max_points, n_channels):
    ...
    frames = [Image.open(os.path.join(d, "frames", f"f{i:02d}.jpg")).convert("RGB")
              for i in rec["frames"]]
    # 태스크마다 보는 크기가 다르다. 번들은 가장 큰 것 하나로 담고 여기서
    # 줄인다 -- 학습의 `instruct_data.FRAME_SIZE` 와 같은 표를 써야 하고,
    # 어긋나면 비전 토큰 수가 달라져 점수가 조용히 무너진다.
    want = FRAME_SIZE.get(rec["task"], DEFAULT_FRAME_SIZE)
    if frames and frames[0].size != want:
        frames = [f.resize(want, Image.BILINEAR) for f in frames]
